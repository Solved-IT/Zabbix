...
#$MYSQL "SELECT host FROM hosts where host LIKE '$1'"
$MYSQL " SELECT ip FROM hosts t1 INNER JOIN interface t2 ON t1.hostid = t2.hostid WHERE ip LIKE '$1' GROUP BY ip" | while read line
#####################################################################################################
...
snmpName=$(snmpwalk -Oqv -v2$ -c public ${arr[0]} system.sysName.0 2> /dev/null)
if [ -z "$snmpName" ]
then
    echo "Sysname is empty, will not update name (host unreachable?)"
else
